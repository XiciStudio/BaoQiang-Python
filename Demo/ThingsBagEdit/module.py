...
def Get_SaveData(uid, gameid, index, format):
    ...
    Verify = Get_md5(
        Get_md5(Get_md5("SDALPlsldlnSLWPElsdslSE" + index + Get_gamekey(gameid) + uid + gameid + "PKslsO")))
    data = {"gameid": gameid, "verify": Verify, "gamekey": Get_gamekey(gameid), "index": index, "uid": uid}
    redata = requests.post(url, data=data)
    if (format == "Yes"):
